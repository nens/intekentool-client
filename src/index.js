			...
			map.length > 0
				? map[0].mapLayers.map((m, i) => {
						return {
							...
							wmsUrl: `${m.url}?service=WMS&request=GetMap&layers=${m.layerName}&width=256&height=256&transparent=true&version=1.1.1&STYLES=${m.styles}&srs=EPSG:3857&REQUEST=GetMap&format=${m.format}&bbox={bbox-epsg-3857}`
						};
					})
				: [];
